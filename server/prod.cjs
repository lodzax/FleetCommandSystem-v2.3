...
app.put('/api/trucks/:id', async (req, res) => { try { const t = req.body; await pool.query('UPDATE trucks SET plateNumber=?,type=?,model=?,capacity=?,status=?,fuelRate=?,currentLat=?,currentLng=?,driverId=?,assignedDriverName=?,mileage=?,nextServiceMileage=?,lastServiceDate=?,imageUrl=?,category=? WHERE id=?', [t.plateNumber,t.type,t.model,t.capacity,t.status,t.fuelRate,t.currentLat,t.currentLng,t.driverId||null,t.assignedDriverName||null,t.mileage,t.nextServiceMileage,t.lastServiceDate,t.imageUrl||null,t.category||null,req.params.id]); res.json({ok:true}); } catch(e) { res.status(500).json({error:e.message}); } });
